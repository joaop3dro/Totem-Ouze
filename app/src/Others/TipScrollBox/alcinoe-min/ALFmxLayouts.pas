...
type
  ...
  // ---------------------------------------------------------------------------
  // [iPub - github.com/viniciusfbb] - 23/08/2021 - Delphi 10.4
  // Better control the visibility of scrollbars
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  ...
  private
    ...
    procedure SetScrollBarsVisibility(const AValue: TScrollBarsVisibility);
    {$ENDIF} // ----------------------------------------------------------------
  ...
  public
    ...
    // Better control the visibility of scrollbars
    {$IFDEF IPUB_FIXES} // -----------------------------------------------------
    property ShowScrollBars: Boolean read GetShowScrollBars write SetShowScrollBars stored False;
...
begin
  ...
  // Better control the visibility of scrollbars
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  FScrollBarsVisibility := TScrollBarsVisibility.Always;
  {$ELSE} // -------------------------------------------------------------------
...
procedure TALCustomScrollBox.DoRealign;
  ...
  begin
    if fVScrollBar <> nil then begin
      ...
      // Better control the visibility of scrollbars
      {$IFDEF IPUB_FIXES} // ---------------------------------------------------
      var LLastVisible: Boolean := fVScrollBar.Visible;
      case FScrollBarsVisibility of
        TScrollBarsVisibility.Always: fVScrollBar.Visible := (AContentRect.Height > Height) or (not FAutoHide);
        TScrollBarsVisibility.OnMouseInteractions:
          begin
            {$IF not defined(iOS) and not defined(ANDROID)}
            fVScrollBar.Visible :=
              ((IsMouseOver and ((not FMouseEvents) or (not FAniCalculations.Averaging))) or
              fVScrollBar.IsMouseOver or fVScrollBar.Thumb.IsMouseOver or fVScrollBar.IsTracking) and
              ((AContentRect.Height > Height) or (not FAutoHide));
            {$ELSE}
            fVScrollBar.Visible := False;
            {$ENDIF}
          end;
        TScrollBarsVisibility.None: fVScrollBar.Visible := False;
      end;
      if fVScrollBar.Visible and (fVScrollBar.Visible <> LLastVisible) then
  ...
  begin
    if fHScrollBar <> nil then begin
      ...
      // Better control the visibility of scrollbars
      {$IFDEF IPUB_FIXES} // ---------------------------------------------------
      var LLastVisible: Boolean := fHScrollBar.Visible;
      case FScrollBarsVisibility of
        TScrollBarsVisibility.Always: fHScrollBar.Visible := (AContentRect.Width > Width) or (not FAutoHide);
        TScrollBarsVisibility.OnMouseInteractions:
          begin
            {$IF not defined(iOS) and not defined(ANDROID)}
            fHScrollBar.Visible :=
              ((IsMouseOver and ((not FMouseEvents) or (not FAniCalculations.Averaging))) or
              fHScrollBar.IsMouseOver or fHScrollBar.Thumb.IsMouseOver or fHScrollBar.IsTracking) and
              ((AContentRect.Width > Width) or (not FAutoHide));
            {$ELSE}
            fHScrollBar.Visible := False;
            {$ENDIF}
          end;
        TScrollBarsVisibility.None: fHScrollBar.Visible := False;
      end;
      if fHScrollBar.Visible and (fHScrollBar.Visible <> LLastVisible) then
...
begin
  ...
  try
    ...
    if (FContent <> nil) then begin
      ...
      // -----------------------------------------------------------------------
      // [iPub - github.com/viniciusfbb] - 09/03/2022 - Delphi 11.0
      // Fixed issue with wrong content position when viewbox is changing
      {$IFDEF IPUB_FIXES} // ---------------------------------------------------
      Content.SetBounds(Content.Position.X + LContentRect.Left + fAnchoredContentOffset.x,
                        Content.Position.Y + LContentRect.Top + fAnchoredContentOffset.y,
                        LContentRect.Width,
                        LContentRect.Height);
      {$ELSE} // ---------------------------------------------------------------
      ...
      if LContentRect.EqualsTo(CalcContentBounds, Tepsilon.Position) then  begin
        ...
        // [iPub - github.com/viniciusfbb] - 06/08/2021 - Delphi 10.4
        // Improving the compatibility with TCustomScrollBox. This is important
        // because in a custom scrollbox we can easily change the parent class
        // of TScrollBox to TALScrollBox
        {$IFDEF IPUB_FIXES} // -------------------------------------------------
...
begin
  ...
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  {$IF not defined(iOS) and not defined(ANDROID)}
  if (FScrollBarsVisibility = TScrollBarsVisibility.OnMouseInteractions) then
  begin
    if Assigned(FHScrollBar) then
    begin
      FHScrollBar.Visible := (not (ssTouch in Shift)) and (FHScrollBar.Enabled or (fAniCalculations.ViewportPosition.X <> 0));
      if FHScrollBar.Visible then
        FHScrollBar.BringToFront;
    end;
    if Assigned(FVScrollBar) then
    begin
      FVScrollBar.Visible := (not (ssTouch in Shift)) and (FVScrollBar.Enabled or (fAniCalculations.ViewportPosition.Y <> 0));
...
// Better control the visibility of scrollbars
{$IFDEF IPUB_FIXES} // ---------------------------------------------------------
function TALCustomScrollBox.GetShowScrollBars: Boolean;
begin
  Result := FScrollBarsVisibility = TScrollBarsVisibility.Always;
end;

procedure TALCustomScrollBox.SetScrollBarsVisibility(const AValue: TScrollBarsVisibility);
begin
  if FScrollBarsVisibility <> AValue then
  begin
...
begin
  if Value then
    ScrollBarsVisibility := TScrollBarsVisibility.Always
  else
    ScrollBarsVisibility := TScrollBarsVisibility.None;
end;
{$ELSE} // ---------------------------------------------------------------------
procedure TALCustomScrollBox.SetShowScrollBars(const Value: Boolean);
begin
