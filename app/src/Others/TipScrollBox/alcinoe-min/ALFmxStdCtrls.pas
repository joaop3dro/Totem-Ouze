...
type
  ...
  {~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}
  TALTrackThumb = class({$IFDEF SKIA}TRectangle{$ELSE}TALRectangle{$ENDIF})
  private
  ...
  // ---------------------------------------------------------------------------
  // [iPub - github.com/viniciusfbb] - 24/08/2021 - Delphi 10.4
  // Expand the scrollbar when mouse over
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  private type
    TALScrollBarTrackThumb = class(TALTrackThumb)
    protected
  ...
  // ---------------------------------------------------------------------------
  // [iPub - github.com/viniciusfbb] - 24/08/2021 - Delphi 10.4
  // Added support for IIsChecked interface
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  TALCheckBox = class(TControl, IIsChecked)
  {$ELSE} // -------------------------------------------------------------------
...
begin
  ...
  // ---------------------------------------------------------------------------
  // [iPub - github.com/viniciusfbb] - 24/08/2021 - Delphi 10.4
  // The default cursor cannot be crHandPoint
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  {$ELSE} // -------------------------------------------------------------------
  cursor := crHandPoint;
  {$ENDIF} // ------------------------------------------------------------------
...
begin
  // ---------------------------------------------------------------------------
  // [iPub - github.com/viniciusfbb] - 24/08/2021 - Delphi 10.4
  // Fixing the mouse tracking to don't be captured by the scrollbox too
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
...
begin
  ...
  // ---------------------------------------------------------------------------
  // [iPub - github.com/viniciusfbb] - 24/08/2021 - Delphi 10.4
  // Expand the scrollbar when mouse over and set a thinner trackbar on mobile
  // than on desktop
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  FThumb := TALScrollBarTrackThumb.Create(self, FvalueRange, False{aWithGlyphObj});
  {$ELSE} // -------------------------------------------------------------------
  ...
  // than on desktop
  {$IFDEF IPUB_FIXES} // -------------------------------------------------------
  FThumb.Stroke.Thickness := 0;
  {$IF defined(ANDROID) OR defined(iOS)}
  fThumb.Fill.Color := $7F000000;
  {$ELSE}
  fThumb.Fill.Color := $33000000;
  {$ENDIF}
  {$ELSE} // -------------------------------------------------------------------
...
{$IFDEF IPUB_FIXES} // ---------------------------------------------------------
{********************************************}
function TALCheckBox.IsCheckedStored: Boolean;
begin
...
// Expand the scrollbar when mouse over
{$IFDEF IPUB_FIXES} // ---------------------------------------------------------
{ TALScrollBar.TALScrollBarTrackThumb }

procedure TALScrollBar.TALScrollBarTrackThumb.DoMouseEnter;
begin
  inherited;
...
procedure TALScrollBar.TALScrollBarTrackThumb.DoMouseLeave;
begin
  inherited;
  {$IF not defined(ANDROID) and not defined(iOS)}
  Fill.Color := $33000000;
...
procedure TALScrollBar.TALScrollBarTrackThumb.SetBounds(X, Y, AWidth,
  AHeight: Single);
begin
  BeginUpdate;
  try
    ...
    if IsMouseOver then
    begin
      if TALCustomTrack(Owner).Orientation = TOrientation.Vertical then
      begin
        X := -2;
        AWidth := 8;
      end
      else if TALCustomTrack(Owner).Orientation = TOrientation.Horizontal then
      begin
        Y := -2;
        AHeight := 8;
      end;
    ...
    {$ENDIF}
    XRadius := System.Math.Min(AWidth, AHeight) / 2;
    YRadius := XRadius;
    inherited;
  finally
    EndUpdate;
