...
uses
  ...
  REST.HttpClient, System.JSON, FMX.Forms
  {$IFDEF MSWINDOWS}
  , WinApi.Windows
  {$ENDIF}
  ;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
         {$IFDEF MSWINDOWS}{$IFDEF DEBUG}OutputDebugString('Logs Pendentes! Recuperando...');{$ENDIF} {$ENDIF}
        LLog := FLogDB.ProximoPendenteAsString(LId);
        if not LLog.Trim.IsEmpty then
        begin
          {$IFDEF MSWINDOWS}{$IFDEF DEBUG}OutputDebugString(PChar('Enviando Log ' + LId.ToString));{$ENDIF} {$ENDIF}
          SendLog(LId, LLog);
          FTentativasEnvio := 0;
        end;
      end;
...
begin
  ...
  try
    ...
    try
      ...
      try
        ...
        {$IFDEF MSWINDOWS}{$IFDEF DEBUG}OutputDebugString('Enviado');{$ENDIF} {$ENDIF}
      finally
        LReq.Free;
      end;
    finally
  ...
  {$IFDEF MSWINDOWS}{$IFDEF DEBUG}OutputDebugString(PChar('Atualizando Log com Id Remoto (' + LRemoteId + ')'));{$ENDIF}{$ENDIF}
  LAtualizou := FLogDB.UpdateSending(AId, LRemoteId);
  {$IFDEF DEBUG}
    if LAtualizou then
      {$IFDEF MSWINDOWS}OutputDebugString(PChar('Log ' + AId.ToString + ' atualizado')){$ENDIF}
    else
     {$IFDEF MSWINDOWS} OutputDebugString(PChar('Falha ao atualizar o log ' + AId.ToString + '. Veja o log de exceções!'));{$ENDIF}
  {$ENDIF}
