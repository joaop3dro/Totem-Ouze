...
uses
  ...
  FireDAC.Phys.SQLiteDef,  Data.DB,  FireDAC.Comp.Client,
  ...
  FireDAC.Phys.SQLiteWrapper.Stat, FireDAC.Comp.UI, FireDAC.VCLUI.Wait;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        result := False;
        TLogSQLiteExceptions.Log('Falha ao adicionar no BD: ' + E.Message, ALog);
      end;
    end;
  finally
...
//  Con.Params.Database := 'C:\Desenvolvimento\Ouze\app\src\db\log.db';//FConfigLog.NomeArquivo;
  Con.Params.Database := FConfigLog.NomeArquivo;
  ConPendentes.Params.Database := Con.Params.Database;
  FUltimoCleanup := Date-1;
  Active := true;//FConfigLog.Active;
