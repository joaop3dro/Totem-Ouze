...
uses System.SysUtils, FMX.Edit, Classes, System.MaskUtils, FMX.StdCtrls, uGosEdit;
...
function StrToBoolean(Info: String):Boolean;

implementation

function StrToBoolean(Info: String):Boolean;
begin
   if strtoint(Info) = 1 then
       Result := True
   else
       Result := False;
end;
...
procedure ResetFormat(Obj: TObject);
begin
    if obj is TGosEditView then
        TGosEditView(obj).Text := SomenteNumero(TGosEditView(obj).Text)
